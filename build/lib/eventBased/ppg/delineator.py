...
class signalProcessing:    
    ...
    def __init__(self, signal = None, windowLength = 2, windowOverlap = 0.7, filt = False, taps = None, filterNorm = None, thStableInit = 10, previousStable = 4):
        ...
        self.labels = dict([['ON',0],['SL',1],['PD',2],['DC',3],['PR',4],['NULLSL',5],['NULLON',6],['NULLPD',7],['NULLDC',8],['NULLPR',9],['NOISE',10],['UD',11]])
        ...
        for w in range(len(waves)):
            ...
            if w in stables:
                for i in range(wv[0],wv[5]):
                    ...
                    #NULL ON --> wait after ONSET
                    if i>wv[0] and i<wv[1]:
                        l = self.labels['NULLON']

                    #NULL SL --> wait after SLOPE
                    if i>wv[1]:
                        if i<wv[2]: #between slope and dominant peack, if no dominant peack is found, no problem: wv[2] >> 0 (default val)
                            l = self.labels['NULLSL']
                        elif wv[2] == 0 and i<wv[3]: #between slope and dicrotic only if no dominant peack is found, no problem no dicrotic exist: wv[4] >> 0 (default val)
                            l = self.labels['NULLSL']
                        elif wv[3] == 0 and i<wv[4]: #between slope and reflected peack only if no dominant peack or dicrotic is found, AT LEAST one of these 3 condition NEED to be true
                            l = self.labels['NULLSL']
                        elif wv[4] == 0:
                            l = self.labels['UD'] #THIS should not be possible NO PEACK WHATSOEVER

                    #NULLPD --> wait after first peack
                    if wv[2] != 0 and i > wv[2]: #no need for the check if == 0, just for readability 
                        if i<wv[3]: #between dominant peack and dicrotic, if no dicrotic is found, no problem: wv[4] >> 0 (default val)
                            l = self.labels['NULLPD']
                        elif wv[3] == 0 and i<wv[4]: #between slope and dicrotic only if no dominant peack is found, no problem no dicrotic exist: wv[4] >> 0 (default val)
                            l = self.labels['NULLPD']
                        elif wv[4] == 0: # After first peack, no other structure (only a inverted U shape)
                            l = self.labels['NULLPD']

                    #NULLDC --> wait after dicrotic
                    if wv[3] != 0 and i > wv[3]: #no need for the check if == 0, just for readability 
                        if i<wv[4]: #between dominant peack and dicrotic, if no dicrotic is found, no problem: wv[4] >> 0 (default val)
                            l = self.labels['NULLDC']
                        elif wv[4] == 0: # After first peack, no other structure (only a inverted U shape)
                            l = self.labels['NULLDC']

                    #NULLRP --> wait after reflected peack
                    if wv[4] != 0 and i > wv[4]: #no need for the check if == 0, just for readability 
                        l = self.labels['NULLPR']

                    seqVals.append(self.signalFilter.v[i])
